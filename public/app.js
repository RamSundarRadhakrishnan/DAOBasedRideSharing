const STORAGE_KEY = "dao-ride-sharing-prototype-v2";
...
function renderUserSummary() {
  ...
  const user = sessionUser();
  if (!user) {
    container.innerHTML = `<div class="stack"><p class="muted">Select a demo user from the login page to load the correct role dashboard.</p></div>`;
    return;
  }
...
function renderLoginView() {
  ...
  return `<section class="grid-two"><article class="content-card"><div class="card-header"><div><p class="eyebrow">Role Based Access Control</p><h3>Choose a user to log in</h3></div></div><div class="notice">Each login lands on a single role-specific page. Navigation only shows pages allowed for that role.</div><div class="list">${Object.entries(groups).map(([role, users]) => `<div class="list-item"><div class="card-header"><div><h3>${safeText(role)}</h3></div></div><div class="list">${users.map(loginCard).join("")}</div></div>`).join("")}</div></article><article class="content-card"><div class="card-header"><div><p class="eyebrow">RBAC Notes</p><h3>What this demonstrates</h3></div></div><div class="list"><div class="list-item">Passenger login opens only the passenger flow plus profile and DAO overview.</div><div class="list-item">Host login opens only the host dashboard plus profile and DAO overview.</div><div class="list-item">Arbitrator and admin users land on their own operational pages.</div><div class="list-item">Role pages are hidden and blocked unless the logged-in user is allowed to access them.</div></div></article></section>`;
}
